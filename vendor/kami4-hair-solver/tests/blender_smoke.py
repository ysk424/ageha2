...
s.collider = body
s.frame_end = 10
s.cache_directory = str(ROOT / "outputs/blender_smoke_cache")
assert bpy.ops.kami4.initialize() == {"FINISHED"}
assert bpy.ops.kami4.simulate() == {"FINISHED"}
assert bpy.ops.kami4.reset() == {"FINISHED"}
assert bpy.ops.kami4.bake() == {"FINISHED"}
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT / "outputs/blender_smoke.blend"))
(ROOT / "outputs/blender_smoke.json").write_text(
    json.dumps(
        dict(
            pass_=True,
            frames=10,
