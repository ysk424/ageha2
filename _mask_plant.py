...
def create_head_mask(
    ...
    if surface_obj is None or surface_obj.type != "MESH":
        raise RuntimeError("ボディは Mesh である必要があります")
    if bpy.data.objects.get("Ageha_HairMask") is not None:
        raise RuntimeError("Ageha_HairMask は既に存在します")
    ...
    if material_index is None:
        raise RuntimeError("マテリアル名に 'Skin_Head' を含むものが見つかりません")
    ...
    try:
        ...
        if uv_source is None:
            raise RuntimeError("ヘッド表面に UV がありません")
        ...
        if not polygons:
            raise RuntimeError("ヘッドマテリアルにポリゴンがありません")
        ...
        mesh = bpy.data.meshes.new("Ageha_HairMask_Mesh")
    ...
    mask_obj = bpy.data.objects.new("Ageha_HairMask", mesh)
    bpy.context.collection.objects.link(mask_obj)
    mask_obj.matrix_world = Matrix.Identity(4)
    mask_obj["ageha_mask_semantics"] = "WHITE=0 cm, BLACK=max length"
    mask_obj["ageha_surface_offset_m"] = offset_m

    image = bpy.data.images.new(
        "Ageha_HairMask_White",
        width=image_size,
        height=image_size,
        alpha=False,
        float_buffer=False,
    ...
    material = bpy.data.materials.new("Ageha_HairMask_Material")
...
def _prepare_empty_curves(curves_obj: bpy.types.Object) -> None:
    if len(curves_obj.data.curves) or len(curves_obj.data.points):
        raise RuntimeError("植えの前に Curves を空にしてください")
...
def plant_mask_hair(
    ...
    if not triangles:
        raise RuntimeError("マスクに塗られた（非白）領域がありません")
...
# keeps the lattice square in both directions while the rows stay flat, which
    # is what the later simulation pass expects.
# ---------------------------------------------------------------------------
...
def plant_z_slice_hair(
    ...
    _write_curves(
        ...
        int_attributes={
            "ageha_z_row": row_ids,
            "ageha_z_strip": strip_ids,
            "ageha_z_order": order_ids,
        },
    )

    curves_obj["ageha_plant_mode"] = "Z_SLICE"
    curves_obj["ageha_z_rows"] = row
    curves_obj["ageha_z_spacing_m"] = spacing
